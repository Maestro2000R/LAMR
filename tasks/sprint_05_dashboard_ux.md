...
### 5.1 Dashboard
- [x] Créer `resources/views/dashboard.blade.php`
...
- [x] Créer `DashboardController` avec une méthode `index()`
- [x] Ajouter la route `/dashboard` et rediriger `/` vers dashboard

### 5.2 Navigation
- [x] Mettre à jour `layouts/app.blade.php` :
...
- [x] Composant Blade `@include('partials.flash')` réutilisable
  - Succès (vert), Erreur (rouge), Info (bleu)
  - Fermeture automatique après 3s (JS minimal)
- [x] Vérifier que tous les contrôleurs utilisent `->with('success', '...')`

### 5.4 UX — Listes
- [x] Pagination sur toutes les listes (15 items par page)
- [x] Message "Aucun résultat" quand la liste est vide
- [x] Confirmation JavaScript sur les boutons "Supprimer"
  ```html
  onclick="return confirm('Supprimer cet élément ?')"
  ```

### 5.5 UX — Formulaires
- [x] Affichage des erreurs de validation inline sous chaque champ
- [x] `old()` pour conserver les valeurs saisies en cas d'erreur
- [x] Bouton de soumission désactivé pendant l'envoi (optionnel)
